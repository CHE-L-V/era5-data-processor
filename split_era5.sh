...
if [ $# -eq 0 ]; then
    ...
    echo "用法: ./split_era5.sh <input_file>"
    echo "Usage: ./split_era5.sh <input_file>"
    exit 1
fi
...
if [[ ! $year =~ ^[0-9]{4}$ ]] || [[ ! $month =~ ^[0-9]{2}$ ]]; then
    echo "错误: 无法从文件名提取有效的年月信息: $filename"
    echo "Error: Cannot extract valid year/month from filename: $filename"
    exit 1
fi
...
for i in $(seq 1 $total_timesteps); do
    ...
    # 生成输出文件名（保存到pl目录）
    output_file="${OUTPUT_DIR}/era5_${year}${month}${day}_${hour}00_pl.nc"
